      ...
      <div className="card">
        ...
        <div style={{display:'flex',gap:12}}>
          <div style={{flex:1}}>
            ...
            <table>
              ...
              <tbody>{materials.map(m=> <tr key={m.code}><td>{m.code} - {m.name}</td><td>{m.stock}</td><td>{m.qcPending}</td><td>{m.rejected}</td></tr>)}</tbody>
            </table>
          </div>
          <div style={{flex:1}}>
        ...
        <div style={{marginTop:12}}>
          ...
          <table className="table-compact">
            ...
            <tbody>{vendors.map(v=> <tr key={v.id}><td>{v.id} - {v.name}</td><td className="muted small">{v.contact}</td><td>{v.classification}</td><td className="muted small">{v.payment}</td></tr>)}</tbody>
          </table>
        </div>
      </div>
    </div>
